...
6. **放樣身體的三角纏繞(winding)其實是反的 → 已改用 `side: THREE.DoubleSide` 解決**。徵狀:身體看起來**半透明/幽靈狀**(可看穿到背景樹木),但頭部球體/腿部圓柱(標準幾何、纏繞正確)卻是實心——一眼就能分辨是放樣獨有的問題。診斷:`mesh.material.side===0`(FrontSide)、`transparent===false` 卻透明 = 近面被剔除、看到內壁。修法:`skinMat` 設 DoubleSide(對封閉的腿/頭無害)。若之後要改回單面以省效能,就把 `loft()` 的 `idx.push(a,c,b,b,c,d)` 反轉成 `idx.push(a,b,c,b,d,c)`。
